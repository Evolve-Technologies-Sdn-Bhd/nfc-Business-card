...
export const useNfcCardStore = defineStore('nfcCard', {
  ...
  getters: {
    hasProSubscription: (state) => {
      return state.subscriptionData?.has_pro_subscription || false
    },
  ...
  actions: {
    ...
    async loadNfcCards() {
      ...
      try {
        ...
        if (error.response?.status === 403 && error.response?.data?.upgrade_required) {
          this.error = 'This feature requires a Pro subscription'
        } else {
    ...
    async activateCard(cardId, activationData) {
      ...
      try {
        const { $api } = useNuxtApp()
        const response = await $api.post(`/nfc-cards/${cardId}/activate`, activationData)
