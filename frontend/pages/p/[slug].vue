<!-- pages/p/[slug].vue -->
<template>
    <div class="min-h-screen" :class="getThemeClasses(profile?.theme || 'minimal')">
        <!-- Loading State -->
        <div v-if="loading" class="min-h-screen flex items-center justify-center bg-secondary-100">
        ...
        <!-- Error State -->
        <div v-else-if="error" class="min-h-screen flex items-center justify-center bg-secondary-100">
            <div class="text-center max-w-md mx-auto px-4">
                <Icon name="heroicons:exclamation-triangle" class="h-16 w-16 text-warning-500 mx-auto mb-4" />
                <h1 class="text-2xl font-bold text-secondary-900 mb-2">Profile Not Found</h1>
                <p class="text-secondary-600 mb-6">The profile you're looking for doesn't exist or has been deactivated.
                </p>
        ...
        <div v-else class="relative">
            <!-- Profile Header -->
            <div :class="getHeaderClasses(profile?.theme || 'minimal')" class="relative overflow-hidden">
                <div class="container mx-auto px-4 py-12 text-center relative z-10">
                    <div class="mb-6">
                        <img :src="profile?.profile_image || '/default-avatar.png'" :alt="profile?.name"
                            class="w-32 h-32 rounded-full object-cover border-4 border-white shadow-xl mx-auto" />
                    </div>
                    <h1 class="text-3xl md:text-4xl font-bold text-white mb-2">{{ profile?.name }}</h1>
                    <p class="text-xl text-white/90 mb-2">{{ profile?.title }}</p>
                    <p v-if="profile?.company" class="text-lg text-white/80 mb-4">{{ profile?.company }}</p>
                    <p v-if="profile?.bio" class="text-white/85 max-w-2xl mx-auto leading-relaxed">{{ profile?.bio }}
                    </p>
                </div>

                <!-- Theme-specific decorations -->
                <div v-if="profile?.theme === 'modern'" class="absolute inset-0 overflow-hidden">
                    <div class="absolute -top-10 -right-10 w-40 h-40 bg-white/10 rounded-full"></div>
                    <div class="absolute -bottom-10 -left-10 w-32 h-32 bg-white/10 rounded-full"></div>
                </div>
                <div v-else-if="profile?.theme === 'creative'" class="absolute inset-0 overflow-hidden">
                    <div
            ...
            <div class="container mx-auto px-4 py-8 max-w-2xl">
                <div class="space-y-6">
                    ...
                    <div v-if="hasContactInfo" class="card animate-fade-in-up">
                        <div class="card-body">
                            <h3 class="text-lg font-semibold text-secondary-900 mb-4 flex items-center">
                                <Icon name="heroicons:envelope" class="h-5 w-5 mr-2 text-blue-600" />
                                Contact Information
                            </h3>
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <a v-if="profile?.email" :href="`mailto:${profile.email}`"
                                    ...
                                    <Icon name="heroicons:envelope" class="h-5 w-5 text-secondary-400 mr-3" />
                                    <div>
                                        <p class="text-sm font-medium text-secondary-900">Email</p>
                                        <p class="text-xs text-secondary-600">{{ profile.email }}</p>
                                    </div>
                                </a>
                                ...
                                <a v-if="profile?.phone" :href="`tel:${profile.phone}`"
                                    ...
                                    <Icon name="heroicons:phone" class="h-5 w-5 text-secondary-400 mr-3" />
                                    <div>
                                        <p class="text-sm font-medium text-secondary-900">Phone</p>
                                        <p class="text-xs text-secondary-600">{{ profile.phone }}</p>
                                    </div>
                                </a>
                                ...
                                <a v-if="profile?.website" :href="profile.website" target="_blank"
                                    ...
                                    <Icon name="heroicons:globe-alt" class="h-5 w-5 text-secondary-400 mr-3" />
                                    <div>
                                        <p class="text-sm font-medium text-secondary-900">Website</p>
                                        <p class="text-xs text-secondary-600">{{ formatUrl(profile.website) }}</p>
                                    </div>
                                </a>

                                <div v-if="profile?.location"
                                    class="flex items-center p-3 rounded-lg border border-secondary-200">
                                    <Icon name="heroicons:map-pin" class="h-5 w-5 text-secondary-400 mr-3" />
                                    <div>
                                        <p class="text-sm font-medium text-secondary-900">Location</p>
                                        <p class="text-xs text-secondary-600">{{ profile.location }}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <!-- Social Links -->
                    <div v-if="profile?.social_links?.length" class="card animate-fade-in-up"
                        style="animation-delay: 0.1s">
                        <div class="card-body">
                            <h3 class="text-lg font-semibold text-secondary-900 mb-4 flex items-center">
                                <Icon name="heroicons:share" class="h-5 w-5 mr-2 text-purple-600" />
                                Connect With Me
                            </h3>
                            <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
                                <a v-for="link in profile.social_links" :key="link.id" :href="link.url" target="_blank"
                                    rel="noopener noreferrer"
                                    @click="trackAction('social_click', { platform: link.platform })"
                                    ...
                                    <Icon :name="getSocialIcon(link.platform)"
                                        class="h-8 w-8 mb-2 group-hover:scale-110 transition-transform"
                                        :class="getSocialColor(link.platform)" />
                                    <span class="text-sm font-medium text-secondary-900">{{ link.platform }}</span>
                                </a>
                            </div>
                        </div>
                    </div>
                    ...
                    <!-- Custom Links -->
                    <div v-if="profile?.custom_links?.length" class="card animate-fade-in-up"
                        style="animation-delay: 0.2s">
                        <div class="card-body">
                            <h3 class="text-lg font-semibold text-secondary-900 mb-4 flex items-center">
                                <Icon name="heroicons:link" class="h-5 w-5 mr-2 text-orange-600" />
                                Important Links
                            </h3>
                            <div class="space-y-3">
                                <a v-for="link in profile.custom_links" :key="link.id" :href="link.url" target="_blank"
                                    rel="noopener noreferrer"
                                    @click="trackAction('custom_link_click', { title: link.title })"
                                    ...
                                    <div class="flex items-center">
                                        <div
                                            ...
                                            <Icon name="heroicons:link" class="h-5 w-5 text-orange-600" />
                                        </div>
                                        <div>
                                            <h4 class="font-medium text-secondary-900">{{ link.title }}</h4>
                                            <p v-if="link.description" class="text-sm text-secondary-600">{{
                                                link.description }}</p>
                                        </div>
                                    </div>
                    ...
                    <!-- Portfolio -->
                    <div v-if="profile?.portfolio?.length" class="card animate-fade-in-up"
                        style="animation-delay: 0.3s">
                        <div class="card-body">
                            <h3 class="text-lg font-semibold text-secondary-900 mb-4 flex items-center">
                                <Icon name="heroicons:photo" class="h-5 w-5 mr-2 text-green-600" />
                                Portfolio
                            </h3>
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div v-for="item in profile.portfolio" :key="item.id"
                                    ...
                                    <div class="aspect-video bg-secondary-100 relative">
                                        ...
                                        <div v-else class="flex items-center justify-center h-full">
                                            <Icon name="heroicons:photo" class="h-8 w-8 text-secondary-400" />
                                        </div>
                                        <div v-if="item.url"
                                            ...
                                            <a :href="item.url" target="_blank" rel="noopener noreferrer"
                                                @click="trackAction('portfolio_click', { title: item.title })"
                                    ...
                                    <div class="p-3">
                                        <h4 class="font-medium text-secondary-900 mb-1">{{ item.title }}</h4>
                                        <p class="text-sm text-secondary-600">{{ item.description }}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <!-- Skills Section -->
                    <div v-if="profile?.skills?.length" class="card animate-fade-in-up" style="animation-delay: 0.4s">
                        <div class="card-body">
                            <h3 class="text-lg font-semibold text-secondary-900 mb-4 flex items-center">
                                <Icon name="heroicons:star" class="h-5 w-5 mr-2 text-yellow-600" />
                                Skills & Expertise
                            </h3>
                    ...
                    <!-- Testimonials -->
                    <div v-if="profile?.testimonials?.length" class="card animate-fade-in-up"
                        style="animation-delay: 0.5s">
                        <div class="card-body">
                            <h3 class="text-lg font-semibold text-secondary-900 mb-4 flex items-center">
                                <Icon name="heroicons:chat-bubble-left-ellipsis" class="h-5 w-5 mr-2 text-indigo-600" />
                                What People Say
                            </h3>
                            <div class="space-y-4">
                                <div v-for="testimonial in profile.testimonials" :key="testimonial.id"
                                    class="p-4 rounded-lg bg-secondary-50 border border-secondary-200">
                                    <p class="text-secondary-700 mb-3 italic">"{{ testimonial.content }}"</p>
                                    <div class="flex items-center">
                                        <img v-if="testimonial.avatar" :src="testimonial.avatar"
                                            :alt="testimonial.author" class="w-8 h-8 rounded-full object-cover mr-3" />
                                        <div v-else
                                            class="w-8 h-8 bg-secondary-300 rounded-full flex items-center justify-center mr-3">
                                            <Icon name="heroicons:user" class="h-4 w-4 text-secondary-600" />
                                        </div>
                                        <div>
                                            <p class="text-sm font-medium text-secondary-900">{{ testimonial.author }}
                                            </p>
                                            <p class="text-xs text-secondary-600">{{ testimonial.position }}</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="text-center pt-8 border-t border-secondary-200">
                        ...
                        <a href="https://nfccard.app" target="_blank" rel="noopener noreferrer"
                            class="inline-flex items-center text-primary-600 hover:text-primary-700 font-medium">
                            <Icon name="heroicons:credit-card" class="h-4 w-4 mr-2" />
                            NFCCard.app
                        </a>
                        <p class="text-xs text-secondary-400 mt-2">Create your own smart NFC business card</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div v-if="showSaveSuccess" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div class="bg-white rounded-lg p-6 max-w-sm mx-4 text-center">
                <div class="w-16 h-16 bg-green-100 rounded-full flex items-center justify-center mx-auto mb-4">
                    <Icon name="heroicons:check" class="h-8 w-8 text-green-600" />
                </div>
                <h3 class="text-lg font-semibold text-secondary-900 mb-2">Contact Saved!</h3>
                <p class="text-secondary-600 mb-4">{{ profile?.name }}'s contact has been saved to your device.</p>
        ...
        <div v-if="showShareModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div class="bg-white rounded-lg p-6 max-w-md mx-4">
                <div class="flex items-center justify-between mb-6">
                    <h3 class="text-lg font-semibold text-secondary-900">Share Profile</h3>
                    <button @click="showShareModal = false" class="text-secondary-400 hover:text-secondary-600">
                ...
                <div class="space-y-4">
                    <div>
                        <label class="block text-sm font-medium text-secondary-700 mb-2">Profile URL</label>
                        <div class="flex items-center space-x-2">
                            <input :value="profileUrl" readonly class="input input-bordered flex-1 text-sm" />
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <button @click="shareVia('whatsapp')" class="btn btn-outline justify-start">
                            <Icon name="simple-icons:whatsapp" class="h-4 w-4 mr-2 text-green-600" />
                            WhatsApp
                        </button>
                        <button @click="shareVia('telegram')" class="btn btn-outline justify-start">
                            <Icon name="simple-icons:telegram" class="h-4 w-4 mr-2 text-blue-600" />
                            Telegram
                        </button>
                        <button @click="shareVia('twitter')" class="btn btn-outline justify-start">
                            <Icon name="simple-icons:twitter" class="h-4 w-4 mr-2 text-blue-400" />
                            Twitter
                        </button>
                        <button @click="shareVia('linkedin')" class="btn btn-outline justify-start">
                            <Icon name="simple-icons:linkedin" class="h-4 w-4 mr-2 text-blue-700" />
                            LinkedIn
                        </button>
                    </div>
                </div>
...
const hasContactInfo = computed(() => {
    return profile.value?.email || profile.value?.phone || profile.value?.website || profile.value?.location
})
...
const profileUrl = computed(() => {
    ...
    return `https://nfccard.app/p/${slug}`
})
...
const loadProfile = async () => {
    try {
        ...
        const response = await $api.get(`/profiles/${slug}`) // Added .get()
        ...
        if (response.success) {
            ...
            // Set SEO meta
            useSeoMeta({
                title: `${profile.value.name} - ${profile.value.title || 'Digital Business Card'}`,
                description: profile.value.bio || `Connect with ${profile.value.name}`,
                ogTitle: `${profile.value.name} - Digital Business Card`,
                ogDescription: profile.value.bio || `Connect with ${profile.value.name}`,
...
const trackAction = async (action, data = {}) => {
    try {
        const { $api } = useNuxtApp()
        await $api.post('/analytics/track', { // Changed to .post()
            slug,
            action,
            data,
