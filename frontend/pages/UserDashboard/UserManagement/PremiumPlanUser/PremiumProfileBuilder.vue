          ...
          <div class="card-body">
              ...
              >
                ...
                <div class="grid grid-cols-1 gap-4" :class="{ 'md:grid-cols-2': group.fields.length > 1 && !group.fields.some(f => f.field_type === 'richtext' || f.field_type === 'textarea' || f.field_type === 'repeater') }">
                  <div
                    v-for="field in group.fields"
                    :key="field.field_key"
                    :class="isFieldRestricted(field.field_key) ? 'relative' : ''"
                  >
                    <!-- Restriction Overlay for Premium Plan -->
                    <div
                      v-if="isFieldRestricted(field.field_key)"
                      class="absolute inset-0 bg-gray-50/90 backdrop-blur-sm rounded-lg z-10 flex items-center justify-center border-2 border-amber-300"
                    >
                      <div class="text-center px-4">
                        <Icon name="heroicons:lock-closed" class="w-8 h-8 mx-auto mb-2 text-amber-600" />
                        <p class="text-sm font-semibold text-gray-900 mb-1">Business Plan Feature</p>
                        <p class="text-xs text-gray-600">Upgrade to access this field</p>
                      </div>
                    </div>
                    <DynamicFormField
                      :field="field"
                      v-model="profileData"
                      :user-plan="authStore.user?.subscription_plan || 'premium'"
              ...
              <div class="bg-gradient-to-r from-purple-50 to-pink-50 border border-purple-200 rounded-xl p-4">
                <div class="flex items-center justify-between">
                  <div class="flex items-center gap-2">
                    ...
                    <span class="text-sm font-medium text-gray-900">Services Added</span>
                  </div>
                  <span class="text-sm font-bold text-purple-600">
                    {{ profileData.services ? profileData.services.length : 0 }} / {{ PREMIUM_MAX_SERVICES }} services
                  </span>
                </div>
              </div>
            ...
            <div v-if="activeTab === 'links'" class="space-y-4 sm:space-y-6">
              ...
              <div class="bg-gradient-to-r from-purple-50 to-pink-50 border border-purple-200 rounded-xl p-4">
                <div class="flex items-center justify-between">
                  <div class="flex items-center gap-2">
                    <Icon name="heroicons:link" class="w-5 h-5 text-purple-600" />
                    <span class="text-sm font-medium text-gray-900">Social Links</span>
                  </div>
                  <span class="text-sm font-bold text-purple-600">
                    {{ links.length }} / {{ PREMIUM_MAX_SOCIAL_LINKS }} links
                  </span>
                </div>
              </div>
            ...
            <div v-if="activeTab === 'portfolio'" class="space-y-4 sm:space-y-6">
              ...
              <!-- Premium: Gallery Images Section -->
              <div class="bg-white border border-gray-200 rounded-xl p-6 space-y-4">
                <div class="flex items-center justify-between">
                  <div class="flex items-center gap-2">
                    <Icon name="heroicons:photo" class="w-5 h-5 text-purple-600" />
                    <h3 class="text-sm font-semibold text-gray-800">Gallery Images</h3>
                  </div>
                  <span class="text-xs text-gray-500 bg-purple-100 px-2 py-1 rounded-full">
                    {{ profileData.galleryImages.length }} / {{ PREMIUM_MAX_GALLERY }} images
                  </span>
                </div>
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 gap-4">
                  <div
                    v-for="(image, index) in profileData.galleryImages"
                    :key="index"
                    class="relative group aspect-square rounded-lg overflow-hidden border-2 border-gray-200 hover:border-purple-400 transition-colors"
                  >
                    <img
                      :src="image"
                  ...
                  <!-- Upload New Image -->
                  <label
                    v-if="profileData.galleryImages.length < PREMIUM_MAX_GALLERY"
                    class="aspect-square rounded-lg border-2 border-dashed border-gray-300 hover:border-purple-400 cursor-pointer flex flex-col items-center justify-center bg-gray-50 hover:bg-purple-50 transition-colors"
                  >
                    <Icon name="heroicons:plus" class="w-8 h-8 text-gray-400 mb-1" />
                    <span class="text-xs text-gray-500">Add Image</span>
                    <input
                      type="file"
                      accept="image/*"
                      class="hidden"
                      @change="addGalleryImage($event)"
                    />
                  </label>
                </div>

                <p class="text-xs text-gray-500 mt-2">
                  💡 Tip: Upload high-quality images (recommended: 1200x1200px, max 5MB each)
                </p>
              </div>
...
const isFieldRestricted = (fieldKey) => {
  return PREMIUM_PLAN_RESTRICTED_FIELDS.includes(fieldKey);
};
...
const profileData = reactive({
  ...
  companySlogan: "",          // Company Slogan (Premium)
  ...
  galleryImages: [],          // Premium: Gallery Images (max 6)
  ...
  viewCount: 0,             // Profile page views (server-updated)
  tapCount: 0,              // NFC card taps (server-updated)
...
const saveLink = async () => {
  try {
    ...
    // Premium Plan: Check social links limit (5 max)
    if (links.value.length >= PREMIUM_MAX_SOCIAL_LINKS) {
      $toast.error(`Premium plan allows maximum ${PREMIUM_MAX_SOCIAL_LINKS} social links. Upgrade to Business for unlimited links.`);
      return;
    }
...
const addService = async () => {
  ...
  // Premium Plan: Check service limit (6 max)
  if (editingServiceIndex.value === null && profileData.services && profileData.services.length >= PREMIUM_MAX_SERVICES) {
    $toast.error(`Premium plan allows maximum ${PREMIUM_MAX_SERVICES} services. Upgrade to Business for unlimited services.`);
    return;
  }
...
// Gallery Image Management (Premium)
const addGalleryImage = async (event) => {
  const file = event.target.files[0];
  if (!file) return;

  // Check gallery limit
  if (profileData.galleryImages.length >= PREMIUM_MAX_GALLERY) {
    $toast.error(`Premium plan allows maximum ${PREMIUM_MAX_GALLERY} gallery images.`);
    return;
  }
  ...
  try {
    ...
    reader.onload = (e) => {
      profileData.galleryImages.push(e.target.result);
...
const removeGalleryImage = (index) => {
  if (profileData.galleryImages && profileData.galleryImages[index]) {
...
const saveProfile = async () => {
  ...
  // Premium Plan: Validate bio length (500 char max)
  if (profileData.bio && profileData.bio.length > PREMIUM_MAX_BIO_LENGTH) {
    $toast.error(`Bio exceeds ${PREMIUM_MAX_BIO_LENGTH} characters. Premium plan limit.`);
    return;
  }

  // Premium Plan: Validate services count (6 max)
  if (profileData.services && profileData.services.length > PREMIUM_MAX_SERVICES) {
    $toast.error(`Premium plan allows maximum ${PREMIUM_MAX_SERVICES} services.`);
    return;
  }

  // Premium Plan: Validate social links count (5 max)
  if (links.value.length > PREMIUM_MAX_SOCIAL_LINKS) {
    $toast.error(`Premium plan allows maximum ${PREMIUM_MAX_SOCIAL_LINKS} social links.`);
    return;
  }

  // Premium Plan: Validate gallery images count (6 max)
  if (profileData.galleryImages && profileData.galleryImages.length > PREMIUM_MAX_GALLERY) {
    $toast.error(`Premium plan allows maximum ${PREMIUM_MAX_GALLERY} gallery images.`);
    return;
  }
...
// Initialize
onMounted(async () => {
  console.log('🟣 PREMIUM PROFILE BUILDER LOADED - New component with restrictions');
  console.log('📋 Premium Plan Limits:', {
    maxServices: PREMIUM_MAX_SERVICES,
    maxLinks: PREMIUM_MAX_SOCIAL_LINKS,
    maxGallery: PREMIUM_MAX_GALLERY,
    maxBio: PREMIUM_MAX_BIO_LENGTH,
    restrictedFields: PREMIUM_PLAN_RESTRICTED_FIELDS
  });
