  ...
  <div>
    ...
    <div class="mb-8">
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 class="text-2xl sm:text-3xl font-bold text-secondary-900">Basic Profile Builder</h1>
          <p class="mt-1 sm:mt-2 text-sm sm:text-base text-secondary-600">
            Customize your digital business card with 8 essential fields
          </p>
          <div class="mt-2 inline-flex items-center gap-2 px-3 py-1 bg-amber-50 border border-amber-200 rounded-lg">
            <Icon name="heroicons:lock-closed" class="w-4 h-4 text-amber-600" />
            <span class="text-xs text-amber-700 font-medium">Basic Plan: 8 fields available</span>
          </div>
        </div>
        ...
        <div class="card">
          <div class="card-body">
                ...
                <div class="grid grid-cols-1 gap-4" :class="{ 'md:grid-cols-2': group.fields.length > 1 && !group.fields.some(f => f.field_type === 'richtext' || f.field_type === 'textarea' || f.field_type === 'repeater') }">
                  ...
                  >
                    <DynamicFormField
                      :field="field"
                      v-model="profileData"
                      :user-plan="authStore.user?.subscription_plan || 'basic'"
                      :disabled="isFieldRestricted(field.field_key)"
                    />
                    <!-- Locked Field Overlay for Basic Plan -->
                    <div
                      v-if="isFieldRestricted(field.field_key)"
                      class="absolute inset-0 bg-gray-50/90 backdrop-blur-sm rounded-lg border-2 border-amber-300 flex items-center justify-center cursor-not-allowed group hover:bg-gray-100/90 transition-all"
                    >
                      <div class="text-center p-4">
                        <Icon name="heroicons:lock-closed-solid" class="w-8 h-8 text-amber-500 mx-auto mb-2" />
                        <p class="text-sm font-semibold text-gray-900 mb-1">{{ field.label }}</p>
                        <p class="text-xs text-gray-600 mb-3">Available in Business Plan</p>
                        <button
                          @click="showUpgradeModal"
                          class="text-xs font-medium text-white bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-600 hover:to-orange-600 px-4 py-2 rounded-full transition-all shadow-md hover:shadow-lg"
                        >
                          Upgrade Now
                        </button>
                      </div>
            ...
            <div v-if="activeTab === 'company'" class="space-y-4 sm:space-y-6">
              <!-- Company Logo (Locked for Basic Plan) -->
              <div class="relative">
                <ProfileImageUpload
                  ...
                  :disabled="isFieldRestricted('companyLogo')"
                />
                <!-- Locked Overlay for Company Logo -->
                <div
                  v-if="isFieldRestricted('companyLogo')"
                  class="absolute inset-0 bg-gray-50/90 backdrop-blur-sm rounded-lg border-2 border-amber-300 flex items-center justify-center cursor-not-allowed group hover:bg-gray-100/90 transition-all"
                >
                  <div class="text-center p-4">
                    <Icon name="heroicons:lock-closed-solid" class="w-10 h-10 text-amber-500 mx-auto mb-2" />
                    <p class="text-sm font-semibold text-gray-900 mb-1">Company Logo</p>
                    <p class="text-xs text-gray-600 mb-3">Available in Business Plan</p>
                    <button
                      @click="showUpgradeModal"
                      class="text-xs font-medium text-white bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-600 hover:to-orange-600 px-4 py-2 rounded-full transition-all shadow-md hover:shadow-lg"
          ...
          <div class="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto transform transition-all">
            ...
            <div class="bg-gradient-to-r from-amber-500 to-orange-500 p-6 text-white relative">
              ...
              <div class="flex items-center gap-3 mb-2">
                <Icon name="heroicons:rocket-launch" class="w-10 h-10" />
                <h2 class="text-2xl font-bold">Upgrade to Business Plan</h2>
              </div>
              <p class="text-amber-50">Unlock all premium features and grow your business</p>
            </div>
            ...
            <div class="p-6 space-y-6">
              ...
              <div class="bg-gray-50 border border-gray-200 rounded-lg p-4">
                <div class="flex items-center justify-between mb-2">
                  <h3 class="text-sm font-semibold text-gray-900">Your Current Plan</h3>
                  <span class="text-xs bg-gray-200 text-gray-700 px-3 py-1 rounded-full">Basic</span>
                </div>
                <p class="text-sm text-gray-600">8 essential fields available</p>
              </div>

              <!-- Business Plan Features -->
              <div class="bg-gradient-to-br from-amber-50 to-orange-50 border-2 border-amber-200 rounded-lg p-5">
                <div class="flex items-center justify-between mb-4">
                  <h3 class="text-lg font-bold text-gray-900">Business Plan</h3>
                  <span class="text-xs bg-green-500 text-white px-3 py-1 rounded-full font-medium">Recommended</span>
                </div>
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-4">
                  <div class="flex items-start gap-2">
                    <Icon name="heroicons:check-circle-solid" class="w-5 h-5 text-green-600 flex-shrink-0 mt-0.5" />
                    <span class="text-sm text-gray-700">Company Logo Display</span>
                  </div>
                  <div class="flex items-start gap-2">
                    <Icon name="heroicons:check-circle-solid" class="w-5 h-5 text-green-600 flex-shrink-0 mt-0.5" />
                    <span class="text-sm text-gray-700">Job Title & Position</span>
                  </div>
                  <div class="flex items-start gap-2">
                    <Icon name="heroicons:check-circle-solid" class="w-5 h-5 text-green-600 flex-shrink-0 mt-0.5" />
                    <span class="text-sm text-gray-700">Company Tagline</span>
                  </div>
                  <div class="flex items-start gap-2">
                    <Icon name="heroicons:check-circle-solid" class="w-5 h-5 text-green-600 flex-shrink-0 mt-0.5" />
                  ...
                  <div class="flex items-start gap-2">
                    <Icon name="heroicons:check-circle-solid" class="w-5 h-5 text-green-600 flex-shrink-0 mt-0.5" />
                    <span class="text-sm text-gray-700">Department/Division</span>
                  </div>
                  <div class="flex items-start gap-2">
                    <Icon name="heroicons:check-circle-solid" class="w-5 h-5 text-green-600 flex-shrink-0 mt-0.5" />
                    <span class="text-sm text-gray-700">Services Offered</span>
                  </div>
                  <div class="flex items-start gap-2">
                    <Icon name="heroicons:check-circle-solid" class="w-5 h-5 text-green-600 flex-shrink-0 mt-0.5" />
                    <span class="text-sm text-gray-700">Staff ID/Code</span>
                  </div>
                  <div class="flex items-start gap-2">
                    <Icon name="heroicons:check-circle-solid" class="w-5 h-5 text-green-600 flex-shrink-0 mt-0.5" />
                    <span class="text-sm text-gray-700">Team Members</span>
                  </div>
                  <div class="flex items-start gap-2 sm:col-span-2">
                    <Icon name="heroicons:check-circle-solid" class="w-5 h-5 text-green-600 flex-shrink-0 mt-0.5" />
                    <span class="text-sm text-gray-700 font-medium">All Basic features + Unlimited fields</span>
                  </div>
                </div>
              </div>
              ...
              <div class="flex flex-col sm:flex-row gap-3">
                <button
                  ...
                  class="flex-1 bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-600 hover:to-orange-600 text-white font-semibold py-3 px-6 rounded-lg transition-all shadow-lg hover:shadow-xl"
                >
                  Upgrade Now
                </button>
                <button
              ...
              <p class="text-xs text-center text-gray-500">
                ...
                Your data is preserved when upgrading - no information will be lost
              </p>
            </div>
          </div>
        </div>
...
// ========================================
// BASIC PLAN FIELD RESTRICTIONS
// ========================================
// BASIC PLAN RESTRICTIONS - Landing Page Features
// Based on official subscription specifications
const BASIC_PLAN_RESTRICTED_FIELDS = [
  'qualification',         // 1. Qualification / Academic Title 🔒
  'tagline',               // 2. Tagline / Motto 🔒 (has bio instead)
  'companyRegistrationNo', // 3. Registered Number (SSM / Biz ID) 🔒
  'companyDepartment',     // 4. Department / Division 🔒
  'staffId',               // 5. Staff ID / Code 🔒
  'companySlogan',         // 6. Company Slogan / Industry line 🔒
  'teamMembers',           // 7. Team Members Section 🔒
  // Note: contactForm, gallery, video, booking, payment, analytics features restricted via feature flags
];

// BASIC PLAN QUANTITY LIMITS
const BASIC_MAX_SERVICES = 3;        // Maximum 3 services
const BASIC_MAX_SOCIAL_LINKS = 2;    // Maximum 2 social media links
const BASIC_MAX_BIO_LENGTH = 250;    // Bio limited to 250 characters

// Helper function to check if field is restricted
const isFieldRestricted = (fieldKey) => {
  return BASIC_PLAN_RESTRICTED_FIELDS.includes(fieldKey);
};
...
const saveLink = async () => {
  try {
    ...
    // BASIC PLAN LIMIT: Maximum 2 social links
    if (links.value.length >= BASIC_MAX_SOCIAL_LINKS) {
      $toast.error(`Basic plan allows maximum ${BASIC_MAX_SOCIAL_LINKS} social links. Upgrade to Premium for up to 5 links.`);
      return;
    }
...
const addService = async () => {
  ...
  // BASIC PLAN LIMIT: Maximum 3 services
  if (editingServiceIndex.value === null && profileData.services && profileData.services.length >= BASIC_MAX_SERVICES) {
    $toast.error(`Basic plan allows maximum ${BASIC_MAX_SERVICES} services. Upgrade to Premium for up to 6 services.`);
    return;
  }
...
const saveProfile = async () => {
  ...
  // Validate services count (3 max for Basic)
  if (profileData.services && profileData.services.length > BASIC_MAX_SERVICES) {
    $toast.error(`You have ${profileData.services.length} services but Basic plan allows maximum ${BASIC_MAX_SERVICES}. Please remove some or upgrade to Premium.`);
    return;
  }

  // Validate social links count (2 max for Basic)
  if (links.value.length > BASIC_MAX_SOCIAL_LINKS) {
    $toast.error(`You have ${links.value.length} social links but Basic plan allows maximum ${BASIC_MAX_SOCIAL_LINKS}. Please remove some or upgrade to Premium.`);
    return;
  }

  // ========================================
  // BASIC PLAN FIELD VALIDATION
  // Clear restricted fields before saving (data preservation)
  // ========================================
  const restrictedDataBackup = {};
  BASIC_PLAN_RESTRICTED_FIELDS.forEach(fieldKey => {
    if (profileData[fieldKey] !== undefined) {
      // Backup the data (for potential upgrade later)
      restrictedDataBackup[fieldKey] = profileData[fieldKey];
      // Clear from save payload
      // Note: We don't actually delete it, the backend should handle this
      // This is just a client-side validation reminder
    }
  });
  
  // Show a subtle reminder if user tries to save restricted fields
  const hasRestrictedData = Object.keys(restrictedDataBackup).some(key => {
    const value = restrictedDataBackup[key];
    return value && (typeof value === 'string' ? value.trim() : value);
  });
  
  if (hasRestrictedData) {
    console.warn('⚠️ Basic Plan: Restricted fields will not be displayed on your card');
  }
...
const navigateToUpgrade = () => {
  ...
  navigateTo('/UserDashboard/SubscriptionManagement');
};
