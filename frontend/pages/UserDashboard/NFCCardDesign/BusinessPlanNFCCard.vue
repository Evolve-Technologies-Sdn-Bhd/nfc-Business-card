        ...
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-sm text-secondary-600">
              ...
              out of <span class="font-bold">{{ cardQuota.total }}</span> cards
              available to order.
            </p>
          </div>
          <div class="text-right">
            ...
            <div class="text-sm text-secondary-500">Available</div>
          </div>
        </div>

        <!-- Warning if quota is low or zero -->
        <div
          v-if="cardQuota.available === 0"
          class="mt-4 p-4 bg-red-50 border border-red-200 rounded-lg"
        >
          <div class="flex items-start">
            <Icon
              name="heroicons:exclamation-triangle"
              class="h-5 w-5 text-red-600 mt-0.5 mr-2 flex-shrink-0"
            />
            <div>
              <p class="text-sm font-medium text-red-800">No quota available</p>
              <p class="text-sm text-red-700 mt-1">
                You have used all your available card quota. Please contact your
                administrator to increase your quota limit.
              </p>
            </div>
          </div>
        </div>
        <div
          v-else-if="cardQuota.available <= 2"
          ...
          <div class="flex items-start">
            ...
            <div>
              ...
              <p class="text-sm text-yellow-700 mt-1">
                You only have {{ cardQuota.available }} card(s) remaining.
                Consider requesting more quota from your administrator.
              </p>
            </div>
          </div>
        </div>
...
onMounted(async () => {
  ...
  // Load card quota for Business Plan users
  await loadCardQuota();
});
...
const goBack = () => {
  router.push("/UserDashboard/BusinessCardManagement");
};

// Load card quota information
const loadCardQuota = async () => {
  try {
    ...
    if (response.success) {
      cardQuota.total = response.data.total_card_quota || 0;
      cardQuota.used = response.data.ordered_cards_count || 0;
      cardQuota.available = response.data.available_card_quota || 0;
    }
  } catch (error) {
    console.error("Load card quota error:", error);
    $toast.error("Failed to load card quota information.");
  }
};
...
const placeOrder = async () => {
  ...
  if (cardQuota.available <= 0) {
    $toast.error(
      "No available card quota. Please contact administrator to increase your quota."
    );
    return;
  }
  ...
  try {
    ...
    if (response.success) {
      $toast.success(
        "NFC Card order placed successfully! Your card will be delivered soon."
      );

      // Redirect to card management page
      await router.push("/UserDashboard/BusinessCardManagement");
    } else {
    ...
    if (error.response?.status === 403 || error.message?.includes("quota")) {
      $toast.error("Insufficient card quota. Please contact administrator.");
    } else {
