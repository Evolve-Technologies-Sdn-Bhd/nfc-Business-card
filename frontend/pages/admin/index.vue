...
<template>
    <div>
        <div class="mb-8">
            ...
            <p class="mt-2 text-secondary-600">Monitor and manage your NFC business card platform</p>
        </div>
                ...
                <div class="mt-4">
                    <div class="flex items-center text-sm">
                        <Icon
                            ...
                            :class="['h-4 w-4 mr-1', stat.trend === 'up' ? 'text-success-500' : 'text-error-500']" />
                        <span :class="[stat.trend === 'up' ? 'text-success-600' : 'text-error-600']">
                            {{ stat.change }}
                        </span>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div class="card">
                <div class="card-header">
                    ...
                    <NuxtLink to="/admin/users" class="text-sm text-primary-600 hover:text-primary-500">
                        View all
                    </NuxtLink>
                </div>
                <div class="card-body">
                    ...
                    <div v-else-if="dashboard?.recent_users?.length" class="space-y-4">
                        <div v-for="user in dashboard.recent_users" :key="user.id" class="flex items-center space-x-3">
                            <img :src="user.profile?.profile_image || '/default-avatar.png'" :alt="user.full_name"
        ...
        <div class="mt-8">
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <div v-else-if="dashboard?.subscription_breakdown?.length"
                        class="grid grid-cols-1 md:grid-cols-4 gap-4">
                        <div v-for="plan in dashboard.subscription_breakdown" :key="plan.subscription_plan"
                            class="text-center p-4 rounded-lg bg-secondary-50">
                            <h4 class="text-lg font-semibold text-secondary-900 capitalize">
                                {{ plan.subscription_plan }}
                            </h4>
                            <p class="text-3xl font-bold text-primary-600">{{ plan.count }}</p>
        ...
        <div class="mt-8">
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <div v-else-if="dashboard?.system_stats" class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div class="text-center">
                            <Icon name="heroicons:server" class="h-8 w-8 mx-auto text-primary-500 mb-2" />
                            <h4 class="text-sm font-medium text-secondary-900">Storage Used</h4>
                            <p class="text-lg font-semibold text-secondary-900">
                                {{ dashboard.system_stats.total_storage_used?.formatted || '0 B' }}
                            </p>
                        </div>
                        <div class="text-center">
                            <Icon name="heroicons:signal" class="h-8 w-8 mx-auto text-primary-500 mb-2" />
                            <h4 class="text-sm font-medium text-secondary-900">Active Sessions</h4>
                            <p class="text-lg font-semibold text-secondary-900">
                                {{ dashboard.system_stats.active_sessions || 0 }}
                            </p>
                        </div>
                        <div class="text-center">
                            <Icon name="heroicons:clock" class="h-8 w-8 mx-auto text-primary-500 mb-2" />
                            <h4 class="text-sm font-medium text-secondary-900">Last Backup</h4>
                            <p class="text-lg font-semibold text-secondary-900">
