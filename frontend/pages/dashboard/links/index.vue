<!-- pages/dashboard/links/index.vue -->
<template>
    <div class="min-h-screen bg-gray-50">
        ...
        <div class="bg-white shadow-sm border-b sticky top-0 z-40">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="flex items-center justify-between h-14 sm:h-16">
                    <div>
                        <h1 class="text-xl sm:text-2xl font-semibold text-gray-900">Link Management</h1>
                        <p class="text-xs sm:text-sm text-gray-600 hidden sm:block">Manage your links and share your
                            profile</p>
                    </div>
                    <div class="flex items-center space-x-2 sm:space-x-4">
                        <div
                            ...
                            <span class="text-xs sm:text-sm font-medium text-gray-700">Your Link:</span>
                            <a :href="profileUrl" target="_blank"
                                class="text-xs sm:text-sm text-blue-600 hover:text-blue-700 font-medium truncate max-w-[150px] sm:max-w-none">
                                {{ shortProfileUrl }}
                            </a>
                            <button @click="copyProfileUrl" class="ml-2 p-1 hover:bg-gray-200 rounded">
                                <Icon name="heroicons:clipboard-document" class="h-4 w-4 text-gray-600" />
                            </button>
                        </div>
                    </div>
                </div>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6">
            <!-- Mobile Profile URL -->
            <div class="sm:hidden mb-4 flex items-center space-x-2 px-3 py-2 bg-white rounded-lg shadow-sm">
                <Icon name="heroicons:link" class="h-4 w-4 text-gray-600 flex-shrink-0" />
                <a :href="profileUrl" target="_blank"
                    ...
                    <div class="bg-white rounded-lg sm:rounded-xl shadow-sm">
                        <div class="p-4 sm:p-6">
                            ...
                            <div v-if="links.length === 0 && !loading" class="text-center py-8 sm:py-12">
                                ...
                                <p class="text-sm text-gray-600 mb-6">Add your first link to get started</p>
                            ...
                            <div v-else-if="!loading">
                                <draggable v-model="links" @end="updateLinkOrder" item-key="id" class="space-y-3"
                                    handle=".drag-handle">
                                    <template #item="{ element: link, index }">
                                        <div class="group relative">
                                            ...
                                            <div :class="[
                                                ...
                                                <div class="flex items-start space-x-3">
                                                    ...
                                                    <div :class="[
                                                        ...
                                                        link.is_active ? getPlatformColor(link.platform) : 'bg-gray-200'
                                                    ]">
                                                        <Icon :name="getPlatformIcon(link.platform)"
                                                            :class="['h-5 w-5', link.is_active ? 'text-white' : 'text-gray-400']" />
                                                    </div>

                                                    <!-- Link Content -->
                                                    <div class="flex-1 min-w-0">
                                                        <input v-model="link.title" type="text" placeholder="Link title"
                                                    ...
                                                    <div class="flex items-center space-x-2">
                                                        <!-- Toggle Switch -->
                                                        <button @click="toggleLink(link)" :class="[
                                            ...
                                            <div v-if="index < links.length - 1"
                                                ...
                                                <button @click="showAddLinkModal = true; insertIndex = index + 1"
                                ...
                                <!-- Add Link Button -->
                                <button @click="showAddLinkModal = true; insertIndex = null"
                            ...
                            <div class="bg-white rounded-2xl sm:rounded-3xl p-4 sm:p-6 shadow-2xl overflow-hidden">
                                <div class="relative z-10">
                                    ...
                                    <div class="space-y-2 sm:space-y-3">
                                        <a v-for="link in activeLinks" :key="link.id" :href="link.url" target="_blank"
            ...
            <div class="bg-white rounded-2xl p-4 sm:p-6 max-w-md w-full max-h-[90vh] overflow-y-auto">
                ...
                <div class="mb-6">
                    <div class="relative">
                        ...
                        <input v-model="searchQuery" type="text" placeholder="URL or App"
                ...
                <div class="space-y-4">
                    ...
                    <div class="grid grid-cols-3 sm:grid-cols-4 gap-3">
                        <button v-for="platform in popularPlatforms" :key="platform.id"
        ...
        <!-- Platform Form Modal -->
        <div v-if="showPlatformForm"
            class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
            <div class="bg-white rounded-2xl p-4 sm:p-6 max-w-md w-full">
                <div class="flex items-center justify-between mb-4 sm:mb-6">
                    <h3 class="text-lg sm:text-xl font-bold text-gray-900">
                        {{ selectedPlatform ? `Add ${selectedPlatform.name}` : 'Add Custom Link' }}
                    </h3>
                ...
                <form @submit.prevent="saveLink" class="space-y-4">
                    <div>
                        ...
                        <input v-model="linkForm.title" type="text"
                    ...
                    <div>
                        ...
                        <input v-model="linkForm.url" type="url"
                            :placeholder="selectedPlatform?.urlExample || 'https://example.com'"
                            class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                            required />
                    </div>

                    <div class="flex items-center space-x-3">
                        <input type="checkbox" v-model="linkForm.is_active" id="activeLink"
                            class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500" />
                        <label for="activeLink" class="text-sm text-gray-700">Make this link active</label>
                    </div>

                    <div class="flex space-x-3 pt-4">
                        <button type="button" @click="closePlatformForm"
...
const updateLink = async (link) => {
    try {
        const { $api } = useNuxtApp()
        await $api.put(`/links/${link.id}`, { // Changed to .put()
            title: link.title,
            url: link.url,
