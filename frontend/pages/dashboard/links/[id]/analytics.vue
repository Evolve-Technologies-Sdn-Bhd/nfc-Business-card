<!-- pages/dashboard/links/[id]/analytics.vue -->
<template>
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="flex items-center justify-between h-16">
                    <div class="flex items-center space-x-4">
                        <div>
                            <h1 class="text-2xl font-semibold text-secondary-900">{{ tagData?.name || 'Link Analytics'
                                }}
                            </h1>
                            <p class="text-sm text-secondary-600">ID: {{ tagData?.nfc_id }}</p>
                        </div>
                    </div>
                    <div class="flex items-center space-x-4">
                        <select v-model="selectedPeriod" @change="loadAnalytics" class="input text-sm">
            ...
            <div v-if="!hasProSubscription" class="mb-6">
                <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
                    <div class="flex items-center">
                        ...
                        <div class="flex-1">
                            <h3 class="text-sm font-medium text-blue-800">Free Plan Analytics</h3>
                            <p class="text-sm text-blue-700 mt-1">
                                You're viewing basic analytics. Upgrade to Pro for detailed insights, device breakdowns,
                ...
                <div class="card p-6">
                    <div class="flex items-center">
                        <div class="ml-4">
                            ...
                            <p class="text-2xl font-semibold text-secondary-900">{{ analytics.totalTaps }}</p>
                            <p class="text-sm text-success-600">+{{ analytics.tapGrowth }}% from last period</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="card p-6">
                    <div class="flex items-center">
                        ...
                        <div class="ml-4">
                            ...
                            <p class="text-2xl font-semibold text-secondary-900">{{ analytics.uniqueVisitors }}</p>
                            <p class="text-sm text-success-600">+{{ analytics.visitorGrowth }}% from last period</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="card p-6">
                    <div class="flex items-center">
                        ...
                        <div class="ml-4">
                            ...
                            <p class="text-2xl font-semibold text-secondary-900">{{ analytics.avgEngagement }}s</p>
                            <p class="text-sm text-success-600">+{{ analytics.engagementGrowth }}% from last period</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="card p-6">
                    <div class="flex items-center">
                        ...
                        <div class="ml-4">
                            ...
                            <p class="text-lg font-semibold text-secondary-900">{{ analytics.topLocation }}</p>
                            <p class="text-sm text-secondary-600">{{ analytics.topLocationTaps }} taps</p>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <div class="card-body">
                        <div v-if="!hasProSubscription"
                            ...
                            <div class="text-center">
                                <Icon name="heroicons:lock-closed" class="h-12 w-12 text-secondary-400 mx-auto mb-2" />
                                <p class="text-secondary-600">Detailed charts available with Pro subscription</p>
                        ...
                        <div v-else class="h-64 bg-secondary-50 rounded-lg flex items-center justify-center">
                            <div class="text-center">
                                <Icon name="heroicons:chart-bar" class="h-12 w-12 text-secondary-400 mx-auto mb-2" />
                                <p class="text-secondary-600">Interactive chart will be displayed here</p>
                                <p class="text-sm text-secondary-500">Integration with Chart.js or similar</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <div v-if="!hasProSubscription" class="text-center py-8">
                            <Icon name="heroicons:lock-closed" class="h-12 w-12 text-secondary-400 mx-auto mb-2" />
                            <p class="text-secondary-600">Device breakdown available with Pro subscription</p>
                            <button @click="upgradeToPro" class="btn btn-primary btn-sm mt-2">
                                <Icon name="heroicons:star" class="h-4 w-4 mr-1" />
                                Upgrade to Pro
                            </button>
                        </div>
                        <div v-else-if="analytics.deviceTypes.length === 0" class="text-center py-8 text-secondary-500">
                            <Icon name="heroicons:device-phone-mobile" class="h-12 w-12 mx-auto mb-2" />
                            <p>No device data available yet</p>
                        </div>
                        <div v-else class="space-y-4">
                            <div v-for="device in analytics.deviceTypes" :key="device.type"
                                ...
                                <div class="flex items-center space-x-3">
                                    ...
                                    <span class="text-sm text-secondary-600 w-12 text-right">{{ device.percentage
                                        }}%</span>
                                </div>
                            </div>
                        </div>
                ...
                <div class="lg:col-span-2">
                    <div class="card">
                        ...
                        <div class="card-body">
                            ...
                            <div v-else-if="!hasProSubscription" class="text-center py-8 text-secondary-500">
                                <Icon name="heroicons:lock-closed" class="h-12 w-12 mx-auto mb-3" />
                                <p class="font-medium">Recent taps available with Pro subscription</p>
                                <button @click="upgradeToPro" class="btn btn-primary btn-sm mt-2">
                                    <Icon name="heroicons:star" class="h-4 w-4 mr-1" />
                                    Upgrade to Pro
                                </button>
                            </div>
                            <div v-else-if="recentTaps.length === 0" class="text-center py-8 text-secondary-500">
                            ...
                            <div v-else class="space-y-3">
                                <div v-for="tap in recentTaps" :key="tap.id"
                                    ...
                                    <div class="flex items-center space-x-3">
                                        ...
                                        <div>
                                            <div class="flex items-center space-x-2">
                                                <span class="text-sm font-medium text-secondary-900">{{ tap.location ||
                                                    'Unknown Location' }}</span>
                                                <span class="text-xs text-secondary-500">•</span>
                                                <span class="text-xs text-secondary-500">{{ tap.device ||
                                                    'Unknown Device' }}</span>
                                            </div>
                                            <p class="text-xs text-secondary-600">{{ formatDateTime(tap.timestamp) }}
                                            </p>
                                        </div>
                                    </div>
                ...
                <div class="space-y-6">
                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            <div v-if="!hasProSubscription" class="text-center py-8">
                                <Icon name="heroicons:lock-closed" class="h-12 w-12 text-secondary-400 mx-auto mb-2" />
                                <p class="text-secondary-600">Geographic data available with Pro subscription</p>
                                <button @click="upgradeToPro" class="btn btn-primary btn-sm mt-2">
                                    <Icon name="heroicons:star" class="h-4 w-4 mr-1" />
                                    Upgrade to Pro
                                </button>
                            </div>
                            <div v-else-if="analytics.topLocations.length === 0"
                                class="text-center py-8 text-secondary-500">
                                <Icon name="heroicons:map-pin" class="h-12 w-12 mx-auto mb-2" />
                                <p>No location data available yet</p>
                            </div>
                            <div v-else class="space-y-3">
                                <div v-for="location in analytics.topLocations" :key="location.name"
                                    class="flex items-center justify-between">
                                    <div class="flex items-center">
                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            <div v-if="!hasProSubscription" class="text-center py-8">
                                <Icon name="heroicons:lock-closed" class="h-12 w-12 text-secondary-400 mx-auto mb-2" />
                                <p class="text-secondary-600">Time analytics available with Pro subscription</p>
                                <button @click="upgradeToPro" class="btn btn-primary btn-sm mt-2">
                                    <Icon name="heroicons:star" class="h-4 w-4 mr-1" />
                                    Upgrade to Pro
                                </button>
                            </div>
                            <div v-else-if="analytics.peakHours.length === 0"
                                class="text-center py-8 text-secondary-500">
                                <Icon name="heroicons:clock" class="h-12 w-12 mx-auto mb-2" />
                                <p>No time data available yet</p>
                            </div>
                            <div v-else class="space-y-3">
                                <div v-for="hour in analytics.peakHours" :key="hour.time"
                                    class="flex items-center justify-between">
                        ...
                        <div class="card-body text-center">
                            <div class="relative w-24 h-24 mx-auto mb-4">
                                <svg class="w-24 h-24 transform -rotate-90" viewBox="0 0 36 36">
                                    <path class="text-secondary-200" stroke="currentColor" stroke-width="3" fill="none"
                                        d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" />
                                    <path class="text-primary-600" stroke="currentColor" stroke-width="3" fill="none"
                                        stroke-linecap="round" :stroke-dasharray="`${analytics.performanceScore}, 100`"
                                        d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" />
                                </svg>
                                <div class="absolute inset-0 flex items-center justify-center">
                                    <span class="text-xl font-bold text-secondary-900">{{ analytics.performanceScore
                                        }}%</span>
                                </div>
                            </div>
