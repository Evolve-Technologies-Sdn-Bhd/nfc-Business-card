...
class OnboardingController extends Controller
{
    ...
    public function selectPlan(Request $request)
    {
        ...
        try {
            ...
            // Create NFC card for paid plans (basic, premium, business)
            $nfcCard = null;
            if (in_array($plan, ['basic', 'premium', 'business'])) {
                // Check if user already has an active NFC card
                $existingCard = $user->activeNfcCard;
                
                if (!$existingCard) {
                    // Create the NFC card with minimal required fields
                    // The card_id will be auto-generated by the model's boot method
                    $nfcCard = NfcCard::create([
                        'user_id' => $user->id,
                        'card_owner' => $user->full_name,
                        'billing_address' => $user->company ?? 'Not provided',
                        'contact_number' => $user->phone ?? 'Not provided',
                        'purchase_date' => now(),
                        'subscription_plan' => $plan,
                        'status' => 'active',
                        'purchase_amount' => 0, // Will be updated during payment
                    ]);

                    Log::info("Created NFC card for user {$user->id} with plan {$plan}", [
                        'nfc_card_id' => $nfcCard->id,
                        'card_id' => $nfcCard->card_id
                    ]);
                } else {
                    // Update existing card's plan type
                    $existingCard->update(['subscription_plan' => $plan]);
                    $nfcCard = $existingCard;
                    
                    Log::info("Updated existing NFC card for user {$user->id} with plan {$plan}", [
                        'nfc_card_id' => $existingCard->id
                    ]);
                }
            }
