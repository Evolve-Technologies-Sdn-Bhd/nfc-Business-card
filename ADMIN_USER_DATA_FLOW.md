...
3. [Business Plan Quota Data Flow](#3-business-plan-quota-data-flow) ⭐ NEW
4. [Subscription & NFC Card Management Flow](#4-subscription--nfc-card-management-flow)
5. [Profile & Analytics Data Flow](#5-profile--analytics-data-flow)
...
## 3. Business Plan Quota Data Flow

### 3.1 Admin Creates Business User with Quota

```
Admin Panel                      Backend (Laravel)              Database
│                                │                              │
├─ POST /api/admin/users         │                              │
│  {                             │                              │
│    first_name: "ABC Corp",     │                              │
...
│                                ├─> Validate admin role        │
│                                ├─> Validate input             │
│                                │   - require quota if business│
│                                │                              │
│                                ├─> Create user ──────────────>│ INSERT users
│                                │   - subscription_plan='business' │ - first_name, email
│                                │   - parent_business_id=NULL  │ - subscription_plan
│                                │   - total_account_slots=50   │ - parent_business_id: NULL
│                                │   - total_card_quota=50      │ - total_account_slots: 50
│                                │                              │ - total_card_quota: 50
│                                │                              │
...
### 3.2 Business Employee Management - Load Quota

```
BusinessEmployeeManagement.vue   Backend                        Database
(UserDashboard)                  │                              │
│                                │                              │
├─ onMounted()                   │                              │
│  GET /api/business/employees   │                              │
│                                │                              │
│                                ├─> Auth check ───────────────>│ SELECT * FROM users
│                                │   - user is Business         │   WHERE id = ?
│                                │   - parent_business_id=NULL  │
│                                │                              │
│                                ├─> Get employees ────────────>│ SELECT * FROM users
...
│    {                           │                              │
│      employees: [...],         │                              │
│      total_slots: 50,          │ ← From user.total_account_slots
│      total_card_quota: 50,     │ ← From user.total_card_quota
│      used_card_quota: 11,      │ ← Calculated: 1 + count
│      available_card_quota: 39  │ ← Calculated: total - used
│    }                           │                              │
│                                │                              │
...
│  totalCardQuota = 50           │                              │
│  usedCardQuota = 11            │                              │
│  availableCardQuota = 39       │                              │
│                                │                              │
├─ Display UI Cards:             │                              │
│  📊 Account Slots: 39 / 50     │                              │
│  💳 NFC Card Quota: 39 / 50    │                              │
│  👥 Total Employees: 10        │                              │
│  ✅ Active Accounts: 8         │                              │
```

### 3.3 Business Card Management - Load Quota

```
BusinessCardManagement.vue       Backend                        Database
(UserDashboard)                  │                              │
│                                │                              │
├─ onMounted()                   │                              │
...
│                                │   - parent_business_id=NULL  │
│                                │                              │
│                                ├─> Count employees ──────────>│ SELECT COUNT(*) FROM users
│                                │                              │   WHERE parent_business_id = ?
│                                │<─ employee_count = 10        │
│                                │                              │
│                                ├─> Calculate quota:           │
...
│                                │   available = 50 - 11 = 39   │
│                                │                              │
│<── 200 OK                      │<─ Return quota data          │
│    {                           │                              │
│      total_card_quota: 50,     │ ← From user.total_card_quota
│      used_card_quota: 11,      │ ← Calculated
│      available_card_quota: 39, │ ← Calculated
│      employee_count: 10        │ ← Counted
│    }                           │                              │
│                                │                              │
├─ Update cardQuotaInfo          │                              │
│  total: 50                     │                              │
│  used: 11                      │                              │
│  available: 39                 │                              │
│  employees: 10                 │                              │
│                                │                              │
├─ Display Business Card Quota:  │                              │
│  ┌─────────────────────────┐   │                              │
│  │ Business Card Quota     │   │                              │
│  ├─────────────────────────┤   │                              │
│  │ Available: 39 / 50      │   │                              │
│  │ Used: 11                │   │                              │
│  │ Employees: 10           │   │                              │
│  │ ℹ️ 1 admin + 10 emp = 11│   │                              │
│  └─────────────────────────┘   │                              │
```

### 3.4 Order Card with Quota Validation

```
User Orders Business Card        Backend                        Database
│                                │                              │
├─ submitOrder()                 │                              │
│  POST /api/nfc-cards           │                              │
│  {                             │                              │
│    subscription_plan: "business", │                          │
│    check_card_quota: true,     │                              │
│    parent_business_id: null    │ (if Business account)       │
│  }                             │                              │
│                                │                              │
│                                ├─> Authorize Business Plan   │
│                                │   - isBusinessAccount?       │
│                                │   - isBusinessEmployee?      │
│                                │   → REJECT if neither        │
│                                │                              │
│                                ├─> Get Business Account ─────>│ SELECT * FROM users
│                                │   (self or parent)           │   WHERE id = ?
│                                │                              │
│                                ├─> Count employees ──────────>│ SELECT COUNT(*) FROM users
│                                │                              │   WHERE parent_business_id = ?
│                                │<─ employee_count = 10        │
...
│                                │                              │
│                                ├─> Check quota:               │
│                                │   IF used >= total_quota (50)│
│                                │   → REJECT with 400          │
│                                │                              │
│                                ├─> Create order ─────────────>│ INSERT nfc_cards
│                                │   - user_id (orderer)        │   - user_id
│                                │   - business_account_id      │   - business_account_id
│                                │   - subscription_plan        │   - subscription_plan: 'business'
│                                │                              │
│<── 201 Created                 │<─ Order success              │
│    {nfc_card, success}         │                              │
│                                │                              │
├─ Reload quota:                 │                              │
│  await loadCardQuota()         │                              │
│  → Display updated: 38 / 50    │                              │
│                                │                              │
├─ Redirect to NFCCardDesign     │                              │
│  /BusinessPlanNFCCard          │                              │
```
...
```
┌──────────────────────────────────────────────────────────────┐
│ 1. Admin creates Business user                               │
│    ↓                                                         │
│    Sets: total_account_slots = 50, total_card_quota = 50    │
└──────────────────────────────────────────────────────────────┘
                         ↓
┌──────────────────────────────────────────────────────────────┐
│ 2. Data stored in database (users table)                    │
│    • subscription_plan = 'business'                          │
│    • parent_business_id = NULL                               │
│    • total_account_slots = 50                                │
│    • total_card_quota = 50                                   │
└──────────────────────────────────────────────────────────────┘
         ...
         ↓                               ↓
┌─────────────────────┐      ┌─────────────────────┐
│ BusinessEmployee    │      │ BusinessCard        │
│ Management          │      │ Management          │
├─────────────────────┤      ├─────────────────────┤
│ GET /business/      │      │ GET /business/      │
│     employees       │      │     card-quota      │
│                     │      │                     │
│ Returns:            │      │ Returns:            │
│ • total_slots: 50   │      │ • total: 50         │
│ • total_quota: 50   │      │ • used: 11          │
│ • used: 11          │      │ • available: 39     │
│ • available: 39     │      │ • employees: 10     │
│                     │      │                     │
│ Displays:           │      │ Displays:           │
│ 📊 Slots: 39/50     │      │ 💳 Cards: 39/50     │
│ 💳 Quota: 39/50     │      │ 📈 Used: 11         │
└─────────────────────┘      └─────────────────────┘
```
...
## 4. Subscription & NFC Card Management Flow
...
│
└─> nfc_tags (1:N) - nfc_id (chip UID, unique) - nfc_card_id (FK -> nfc_cards.nfc_card_id, nullable)
...
│ GET /tap/{nfc_id} │ │
│ │ │
│ ├─> Find nfc_tags ────────────>│ SELECT _ FROM nfc_tags
...
│ │ │
│ ├─> Get user's profile ───────>│ SELECT _ FROM profiles
...
│ │ │
│ ├─ Query 1: Website Views ────>│ SELECT COUNT(_) FROM analytics
│ │ (profile_view + nfc_tap) │ WHERE user_id = ?
│ │ │ AND action IN ('profile_view','nfc_tap')
│ │ │ AND created_at >= ?
│ │ │
│ ├─ Query 2: Card Taps ────────>│ SELECT COUNT(_) FROM analytics
│ │ │ WHERE user_id = ?
│ │ │ AND action = 'nfc_tap'
│ │ │
...
│ │ │ COUNT(DISTINCT session_id) _ 100
│ │ │ FROM analytics
│ │ │ WHERE user_id = ?
│ │ │
│ ├─ Query 5: Top Links ────────>│ SELECT link_id, link_type, COUNT(_) as clicks
│ │ │ FROM analytics
